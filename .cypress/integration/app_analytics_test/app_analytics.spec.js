...
describe('Creating application', () => {
  ...
  it('Saves current input on reload', () => {
    ...
    cy.wait(delay*3);//Dropdown menu needs to work properly
...
describe('Viewing application', () => {
  ...
  it('Opens service detail flyout when Service Name is clicked', () => {
    ...
    // let getter = cy.get('button[data-test-subj="spanId-link"]').eq(0).focus();
    // cy.wait(1000)
    // getter.click({force:true})
    // getter.click({force:true})
    cy.get('button[data-test-subj="spanId-link"]').eq(0) // dead do not enter
      .scrollIntoView()
      .should('be.visible')
      .should('be.enabled')
      .click();
