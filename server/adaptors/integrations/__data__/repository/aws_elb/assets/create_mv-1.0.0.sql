...
SELECT
    ...
    split_part (target_ip, ':', 1) as `aws.elb.target_ip`,
    split_part (target_ip, ':', 1) as `aws.elb.backend.ip`,
    split_part (target_ip, ':', 1) as `aws.elb.destination.ip`,
    split_part (target_ip, ':', 2) as `communication.destination.port`,
    split_part (target_ip, ':', 2) as `aws.elb.target_port`,
    split_part (target_ip, ':', 2) as `aws.elb.backend.port`,
    split_part (target_ip, ':', 2) as `aws.elb.destination.port`,
    request_processing_time as `aws.elb.request_processing_time`,
    target_processing_time as `aws.elb.target_processing_time`,
    target_processing_time as `aws.elb.backend.processing_time`,
    ...
    target_status_code as `aws.elb.backend.status_code`,
    ...
    split_part (split_part (split_part (request, ' ', 2), '/', 3), ':', 2) as `url.port`,
    split_part (split_part (split_part (request, ' ', 2), '/', 3), ':', 2) as `aws.elb.http.port`,
    split_part (request, ' ', 3) as `url.scheme`,
    split_part (request, ' ', 3) as `aws.elb.http.version`,
