...
import { clearParagraphs, createParagraphs, deleteParagraphs, updateFetchParagraph, updateRunFetchParagraph } from '../../adaptors/notebooks/saved_object_paragraphs';
import {
  DefaultNotebooks,
  DefaultParagraph,
...
export function registerParaRoute(router: IRouter) {
  ...
  router.post(
    ...
    async (
      ...
      try {
        const runResponse = await updateRunFetchParagraph(request.body, context.core.savedObjects.client)
        return response.ok({
          body: runResponse,
        });
      } catch (error) {
  ...
  router.put(
    ...
    async (
      ...
      try {
        const saveResponse = await updateFetchParagraph(request.body, context.core.savedObjects.client)
        return response.ok({
          body: saveResponse,
        });
      } catch (error) {
  ...
  router.post(
    ...
    async (
      ...
      try {
        ...
        const updateResponse = await opensearchNotebooksClient.update(NOTEBOOK_SAVED_OBJECT, request.body.noteId, updateNotebook)
        return response.ok({
          body: updateResponse,
        });
      } catch (error) {
