...
import { TracesTableProps } from './traces_content';

export function DataPrepperTracesTable(props: TracesTableProps) {
  const { items, refresh, loading, traceIdColumnAction } = props;
    ...
    return [
      ...
      },
      {
        ...
        render: (item) =>
          item ? (
            <EuiText size="s">
              {item.length < 36 ? item : <div title={item}>{_.truncate(item, { length: 36 })}</div>}
            </EuiText>
          ) : (
            '-'
          ),
