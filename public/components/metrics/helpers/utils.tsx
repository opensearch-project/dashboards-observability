...
export const pplServiceRequestor = async (pplService: PPLService, finalQuery: string) => {
  const fetchRes = await pplService
    .fetch({ query: finalQuery, format: VISUALIZATION })
    .catch((error: Error) => {
      console.error(error);
    });
  console.log('pplServiceRequestor', JSON.stringify({ finalQuery, fetchRes }));
  return fetchRes;
};
