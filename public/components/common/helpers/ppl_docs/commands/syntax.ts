...
Commans cna be in any order. For example, in the following query, the \`search\` command referencesthe \`accounts\` index as the source, then uses fields and a \`where\` command to do further processing. 
