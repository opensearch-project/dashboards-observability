...
-   \`keepempty\`: Optional. If set to \`true\`, keep the document if any field in the \`field-list\` is null or missing. **Default:** \`false\`
...
### Limitation
The \`dedup\` command is not rewritten to OpenSearch DSL. It's only executed on the coordination node.
`;